...
export class TextProcessor {
  private static questionPatterns = {
    // Question patterns - support various formats
    questionNumber: /^(?:Q|Question)?(\d+)[\.:\)]?\s*(.+)$/i,
    // Option patterns - handle checkmarks and various formats
    optionA: /^A[\.:\)]\s*(.+?)(?:\s*[✓*]|$)/i,
    optionB: /^B[\.:\)]\s*(.+?)(?:\s*[✓*]|$)/i,
    optionC: /^C[\.:\)]\s*(.+?)(?:\s*[✓*]|$)/i,
    optionD: /^D[\.:\)]\s*(.+?)(?:\s*[✓*]|$)/i,
    ...
    multiOptions: /([A-D])[\.:\)]\s*([^A-D]+?)(?=\s*[✓*]|\s+[A-D][\.:\)]|$)/gi,
    // Correct answer indicators
    correctAnswer: /[✓*]/g,
  };

  /**
   * Process text file and extract quiz data
   */
  static async processTextFile(file: File): Promise<ExtractedQuiz> {
    if (typeof window === 'undefined') {
      throw new Error('Text processing is only available on the client side');
    }

    try {
      console.log('Processing text file:', file.name, 'Size:', file.size);
      ...
      const result = this.parseQuizText(text, file.name);
  ...
  private static parseQuizText(text: string, fileName: string): ExtractedQuiz {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (questionMatch) {
        ...
        // Validate and save previous question
      ...
      if (currentQuestion) {
        ...
        if (optionAMatch) {
          ...
          const multiOptionMatch = trimmedLine.match(/([A-D])[\.:\)]\s*([^A-D]+?)(?=\s*[✓*]|\s+[A-D][\.:\)]|$)/gi);
  ...
  /**
   * Validate individual question during parsing
   */
  ...
  static validateQuiz(quiz: ExtractedQuiz): { isValid: boolean; errors: string[] } {
    ...
    if (quiz.questions.length === 0) {
      errors.push('No questions found in text file');
    }
